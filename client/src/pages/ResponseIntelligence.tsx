...
import { BellRing, Check, ClipboardCheck, ClipboardList, Inbox, UsersRound } from "lucide-react";
...
export function WorkspaceSettings() {
  ...
  return <div className="space-y-8">
    ...
    <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
      ...
      <section className="rounded-[28px] border border-[#dce7e7] bg-white p-7 lg:col-span-2"><UsersRound className="h-6 w-6 text-[#0e867e]" /><h2 className="mt-5 text-xl font-extrabold tracking-[-0.045em]">People & access</h2><p className="mt-2 text-sm leading-6 text-[#486170]">Every person keeps a clear role. Invitations will be added with the authentication and notification configuration; current access is visible and owner-managed here.</p><div className="mt-5 divide-y divide-[#e9f0f0] rounded-2xl border border-[#e9f0f0]">{members.isLoading ? <div className="h-16 animate-pulse bg-[#fbfaf7]" /> : members.data?.map(row => <div key={row.member.id} className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold">{row.user.name || row.user.email || "Workspace member"}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-[0.14em] text-[#486170]">{row.user.email || "Account email not available"}</p></div>{row.member.role === "owner" ? <span className="inline-flex h-9 items-center rounded-lg bg-[#e9f0f0] px-3 text-xs font-bold text-[#486170]">Owner</span> : <select disabled={!isOwner || updateRole.isPending} value={row.member.role} onChange={event => updateRole.mutate({ userId: row.member.userId, role: event.target.value as "admin" | "manager" | "analyst" | "responder" | "viewer" })} className="h-9 rounded-lg border border-[#cfdcdd] bg-[#fbfaf7] px-3 text-xs font-bold outline-none focus:border-[#0e867e] disabled:opacity-60"><option value="admin">Admin</option><option value="manager">Manager</option><option value="analyst">Analyst</option><option value="responder">Responder</option><option value="viewer">Viewer</option></select>}</div>)}</div>{!isOwner && <p className="mt-3 text-xs text-[#486170]">Only the workspace owner can change roles.</p>}</section>
    </div>
  </div>;
}
